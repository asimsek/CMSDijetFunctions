...
public:
   double DoEvalPar(double x,const double* p) const
   {
     ...
     double pdf = pow(pow(1-x/p[0],1.0/3.0),p[1])/pow(x/p[0],p[2]+p[3]*log(x/p[0]));
     double eff = 1.;
   ...
   double DoEval(double x) const
   {
     ...
     double pdf = pow(pow(1-x/pars[0],1.0/3.0),pars[1])/pow(x/pars[0],pars[2]+pars[3]*log(x/pars[0]));
     double eff = 1.;
